...
public class PoloniexQuoteReader {
...
public void init(){
	...
	QuoteEventPublisher eventPublisher = new QuoteEventPublisher("poloniex", new TransformPoloniexQuote());
	eventPublisher.start();
	
	//start rest timer/poller. specify input queue and how to get the rest data
	RestPoller restPoller = new RestPoller(eventPublisher.getInputQueue(),delayInMilliSeconds,delayInMilliSeconds,new PoloniexRestDataTask());
	restPoller.start();
	}
